...
const ReactLogo: React.FC = () => (
  <svg className="w-5 h-5 shrink-0 text-[#61DAFB]" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    <ellipse cx="50" cy="50" rx="42" ry="16" stroke="currentColor" strokeWidth="4" />
    <ellipse cx="50" cy="50" rx="42" ry="16" stroke="currentColor" strokeWidth="4" transform="rotate(60 50 50)" />
    <ellipse cx="50" cy="50" rx="42" ry="16" stroke="currentColor" strokeWidth="4" transform="rotate(120 50 50)" />
...
const ViteLogo: React.FC = () => (
  <svg className="w-5 h-5 shrink-0" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M124.5 19.3L67.1 122.2c-1.3 2.3-4.6 2.3-5.9 0L3.8 19.3c-1.5-2.6.8-5.8 3.7-5.2l56.5 12.3 56.7-12.3c3-.6 5.3 2.6 3.8 5.2z" fill="#BD34FE" />
  </svg>
);

const ReduxLogo: React.FC = () => (
  <svg className="w-5 h-5 shrink-0 text-[#764ABC]" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
const RazorpayLogo: React.FC = () => (
  <svg className="w-5 h-5 shrink-0 text-[#008CFF]" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M22.43 4.47L19.5 22H14.7l1.7-10.2L9.2 22H4.4l7.6-17.53h10.43z" fill="#6DB8F5" />
  </svg>
);
...
const TECH_CATEGORIES: TechCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Razorpay API', logo: RazorpayLogo },
    ],
  },
  {
    category: 'DATABASE',
...
export const SliceItTechStack: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="w-full max-w-[880px] space-y-6">
      <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-2">
        {TECH_CATEGORIES.map((cat, idx) => (
          <div
            ...
            className="space-y-3"
          >
            <span className="font-mono text-xs font-bold text-[#6DB8F5] uppercase tracking-wider block border-b border-[#1B3047]/40 pb-1.5">
              {cat.category}
            </span>
            ...
            <div className="flex flex-col space-y-2">
              {cat.items.map((item) => {
                ...
                return (
                  <div
                    key={item.name}
                    className="group flex items-center gap-2.5 py-1 px-1.5 rounded transition-all duration-200 hover:bg-[#6DB8F5]/5 cursor-default"
                  >
                    <div className="transition-transform duration-200 group-hover:scale-110">
                      <LogoComponent />
                    </div>
                    <span className="font-sans text-xs sm:text-sm text-[#A7B5C7]/90 font-medium transition-colors duration-200 group-hover:text-[#E8EEF5]">
                      {item.name}
                    </span>
                  </div>
                );
