...
import { ExternalLink, ArrowRight, ArrowDown, Cpu, Server, Layers, Code, Bot } from 'lucide-react';
...
const TECHNICAL_DETAILS = [
  {
    ...
    desc: 'Fine-tuned sequence-to-sequence model optimized for converting long-form text into concise summaries.',
    icon: Bot,
  },
  {
    category: 'BACKEND',
    name: 'FastAPI',
    desc: 'High-performance Python web framework exposing summarization functionality through a structured API endpoint.',
    icon: Server,
  },
  {
    category: 'ML FRAMEWORK',
    name: 'PyTorch',
    desc: 'Deep learning framework handling tensor calculations and efficient model inference execution.',
    icon: Cpu,
  },
  {
    category: 'NLP LIBRARY',
    name: 'Hugging Face Transformers',
    desc: 'Provides T5 tokenizer, pretrained model weights, text preprocessing, and inference generation pipelines.',
    icon: Layers,
  },
  {
    category: 'FRONTEND',
    name: 'HTML • CSS • JavaScript',
    desc: 'Lightweight, interactive client interface communicating with FastAPI endpoints to render dynamic text summaries.',
    icon: Code,
  },
];

export const TextSummarizerMiniProject: React.FC = () => {
  ...
  return (
    <section
      id="text-summarizer-mini-project"
      ref={containerRef}
      className="relative w-full py-8 sm:py-12 px-4 sm:px-8 flex flex-col items-center bg-transparent"
    >
      {/* OPEN EDITORIAL LAYOUT MATCHING MAJOR PROJECTS (NO ENCLOSING CARD BOX!) */}
      <div className="w-full max-w-[880px] mx-auto flex flex-col space-y-10 sm:space-y-12">
        ...
        <div ref={headerRef} className="flex flex-col space-y-4">
          ...
          <p className="font-sans text-sm sm:text-base text-[#A7B5C7]/90 leading-relaxed max-w-3xl">
            An NLP-based text summarization application powered by a fine-tuned T5 model that converts lengthy text into concise summaries through a FastAPI-powered interface.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs font-mono border-t border-[#1B3047]/40 pt-4">
            ...
            <div className="flex flex-col space-y-0.5">
              ...
              <a
                href="https://github.com/HarshadaBodkhe/Text-Summarizer"
                target="_blank"
        ...
        {/* 2. Workflow Pipeline (Open Flow Container) */}
        <div className="space-y-4 pt-2 border-t border-[#1B3047]/40">
          <div className="space-y-1">
            ...
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-[#E8EEF5]">
              Sequential Summarization Process
            </h2>
          </div>
        ...
        <div className="space-y-4 border-t border-[#1B3047]/40 pt-8">
          ...
          <div ref={techDetailsRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
        ...
        {/* 4. Verified Tech Stack Tags (Open Tag Layout like Major Projects) */}
        ...
        {/* Section Separator between Mini Projects */}
        <div className="w-full border-b border-[#1B3047]/60 pt-6" />
      </div>
    </section>
  );
};
